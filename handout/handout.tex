...
Here, the \s{def} line declares the name of the function and its arguments,
and the \s{return} statement tells the function to exit and yield the
...
You can put a \s{return} statement anywhere in a function you want. One way to
...
By using \s{return} statements judiciously, we can ``pick off'' cases
...
\i{If you have extra time:} what happens when you \s{describe(-1.0)}? Why?
