...
beginners, but we also try to offer some new insights for folks who have done
a little bit of coding before.

Ninety minutes is barely enough time to scratch the surface. We'll try to
teach a few core ideas and set you up to take the next steps on your own.

\vfill

\i{Note for teachers:} This introduction is aimed at \i{version~2} of Python,
not version~3. Students are expected to have a laptop available that can run
the interactive \href{https://jupyter.org/}{Jupyter} notebook. The session is
divided into seven topics, each of which has a ``mini-lecture'' component and
a hands-on component. Each activity is intended to take about 15~minutes to
complete.
...
First things first. Here we'll make sure that you can start Jupyter and run a
few commands in a Jupyter notebook.

\begin{action}
  This formatting calls out an action that you should take as part of the lab.

  Your first task: start Jupyter and create a new, empty notebook. You
  should've already seen how to do this --- if not, or if something goes
  wrong, ask for help from a neighbor or a helper.
\end{action}

The following kind of formatting shows code to be typed into your notebook:
...
Whenever you type code, enter it \i{exactly} as shown --- be careful with
...
\begin{action}
  Run your first code by hitting \s{Shift-Enter}. Check with a neighbor or
  helper if the message \s{Hello, world!} doesn't appear in your notebook.
\end{action}
...
separate cell, using \s{Shift-Enter} to run each cell separately. So here
...
do'' coding. For instance \ldots\ type and run the following. \i{Be careful
  with the ``indentation'' after the \s{while} keyword}:
