...
Start by opening a new terminal window. \typeit Run the command \s{pudb SCRIPT.py}.
...
``\s{print\_function}'' ``\s{\_\_future\_\_} import,'' which lets you use the
\s{print()} function in Python~2 code, and use it everywhere you can. For
annoying technical reasons, that's not a good solution for today, so we stick
with the old-fashioned statement form of \s{print}.
...
is tempting to just write statements like \s{print x, y, z}, but take it from
